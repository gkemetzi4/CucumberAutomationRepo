...
public class DataTableStepDef {
	...
	public void following_data_entered(DataTable table) throws InterruptedException {
		
		List<List<String>> data = table.raw();
		
		String firstName1 = data.get(1).get(0);
		String surname1 = data.get(1).get(1);
		String mobile1 = data.get(1).get(2);
		
		WebDriverWait wait = new WebDriverWait(driver, 5);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.name("firstname")));
		
		driver.findElement(By.name("firstname")).sendKeys(firstName1);
		driver.findElement(By.xpath("//*[@id=\"u_2_d\"]")).sendKeys(surname1);
		driver.findElement(By.id("u_2_g")).sendKeys(mobile1);
		Thread.sleep(2000);
